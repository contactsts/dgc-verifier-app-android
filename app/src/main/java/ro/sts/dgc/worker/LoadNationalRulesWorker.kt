...
@HiltWorker
class LoadNationalRulesWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        Timber.d("National Rules loading start")
        return try {
            val config = configRepository.local().getConfig()
            nationalRulesRepository.loadRules(config.getNationalRulesUrl())
            Timber.d("National Rules loading succeeded")
            Result.success()
        } catch (error: Throwable) {
            Timber.d(error, "National Rules Loading Error: $error")
            Result.retry()
        }
    }

    companion object {
        const val WORK_NAME = "LoadNationalRulesWorker"
    }
}