 ...
 * Whenever the update is downloaded wrapper will call [AppUpdateView.updateReady].
 * At this point your view should ask if user is ready to restart application.
 * Then call one of the continuation methods: [AppUpdateWrapper.userConfirmedUpdate] or [AppUpdateWrapper.userCanceledUpdate]
 *
 * * Calls [AppUpdateView.updateComplete] when update complete (but you may not receive it as your app is restarted during update installation)
 ...
 * @param flowBreaker Terminates update flow if user has already cancelled an update. Use to not to bother user with update popups
...
fun LifecycleOwner.startFlexibleUpdate(
    ...
    Timber.d("In-App Update starting FLEXIBLE update flow...")
