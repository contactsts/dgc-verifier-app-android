...
interface UpdateFlowBreaker : TimeCancelledStorage {
     ...
     * For example you may implement extra logic on update priority level (set from play market) or staleness.
     * Called _instead_ of [isEnoughTimePassedSinceLatestCancel] when processing available update so you may want to combine both time of last offer and update value at the same time.
     * @param updateInfo
     */
    ...
    companion object {
        ...
        /**
         * Wraps flow breaker with an update value check function [valueCheck] which is called when update info gets available.
         * Combined with last update offer time you may decide if to ask again or even cancel the flexible flow and switch to immediate
 ...
 * @param interval An interval between user cancels the update and the next time he is prompted
 * @param timeUnit Time-units the interval is measured in
 * @param storage Stores time update was cancelled
...
internal class IntervalBreaker(
    ...
    override fun isEnoughTimePassedSinceLatestCancel(): Boolean {
        ...
        return (currentTime - timeCancelled > intervalMillis).also {
            Timber.d(
                "Latest time update cancelled: %d, Current time: %d, Enough time passed: %s",
                timeCancelled,
                currentTime,
                if (it) "yes" else "no"
            )
    ...
    /**
     * This breaker processes last time the prompt was displayed only so we just delegate check to common method.
     * @param updateInfo
     */
...
/**
 * Wraps around another [UpdateFlowBreaker] that checks for time intervals delegating value check to [valueCheck] block
...
internal class ValueCheckBreaker(
    ...
    /**
     * Gets time check from [intervalBreaker] and passes check result along with update info to [valueCheck]
     * @param updateInfo
     */
...
internal class WithPreferences(
    ...
    private val clock: Clock = Clock.SYSTEM
) : TimeCancelledStorage {
    companion object {
        /**
         * SharedPreferences storage key for the time update was cancelled
         */
        const val LATEST_CANCEL_PROPERTY = "latest_time_update_cancelled"
    }
    ...
    override fun saveTimeCanceled() {
        val currentTime = clock.getMillis()
        Timber.d("Saving latest time update cancelled: %d", currentTime)
        storage
            .edit()
