 ...
 * [AppUpdateWrapper] works as a presenter for [AppUpdateView] calling methods appropriate for concrete update flow.
 * All view methods are called within [RESUMED] state to make your view stack happy.
 * The update process may end in calling one of two methods:
 * * [updateComplete] - update check has completed without **critical** errors (although [nonCriticalUpdateError] may be called during the workflow)
 * * [updateFailed] - a critical error has occurred during the update. Say the immediate update fails and the application should be terminated
 */
